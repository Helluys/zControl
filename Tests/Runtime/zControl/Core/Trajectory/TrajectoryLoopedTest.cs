namespace zControl.Core.Trajectory {
	...
	/// <summary>
	/// Test cases for <see cref="Trajectory"/> configured as an closed trajectory (looped).
	/// </summary>
	public class TrajectoryLoopedTest {
		/// <summary>
		/// Initial state.
		/// </summary>
		...
		public void SetUp () {
			...
			trajectory = new TrackedTrajectory<string>(points, pointMatcher, true);
		...
		/// <summary>
		/// Next point index should loop back to the beginning of the trajectory.
		/// </summary>
		[Test]
		public void NextPointIndexLoop () {
			CompleteLoop();
			Assert.AreEqual(0, trajectory.NextPointIndex);
		}
		...
		/// <summary>
		/// Next point should loop back to the first point of the trajectory.
		/// </summary>
		[Test]
		public void NextPointLoop () {
			CompleteLoop();
			Assert.AreSame(A, trajectory.NextPoint);
		}
		...
		/// <summary>
		/// Previous point should loop back to the first point of the trajectory.
		/// </summary>
		[Test]
		public void PreviousPointLoop () {
			CompleteLoop();
		...
		/// <summary>
		/// Update should return <c>true</c> when point matches first point after last point was reached.
		/// </summary>
		[Test]
		...
		/// <summary>
		/// Point matcher function should be called with first point as next point when the trajectory loops.
		/// </summary>
		[Test]
		public void PointMatcherLoop () {
			Mock<TrackedTrajectory<string>.PointMatcher> pointMatcherMock = Mock.Get(pointMatcher);
			CompleteLoop();

			trajectory.Update(D);
			pointMatcherMock.Verify(p => p(D, C, A));
		}
		...
		/// <summary>
		/// Looped trajectory never completes.
		/// </summary>
		[Test]
		public void CompletedLoop () {
			CompleteLoop();
